...
*   **处理**：
    1.  **红绿灯提取**：遍历路线 step，统计 `traffic_lights` 数量。由于百度骑行 API 不直接返回该字段，需通过解析 `instruction` 文本（如正则匹配“红绿灯”、“交通灯”关键词）获取。
    2.  **转弯分析**：计算相邻 step 的角度差，识别 > 160° 的掉头弯 (U-turn) 和 < 45° 的急弯。
    3.  **连续性计算**：寻找最长的一段无红绿灯、无急弯的距离。
    4.  **实时路况融合**：(新增) 调用百度地图驾车规划 API (Driving Route) 作为辅助，获取同路径下的拥堵评估（因为骑行 API 不包含实时路况）。通过对比“畅通耗时”与“实际耗时”或直接读取拥堵路段信息，生成“畅通/缓行/拥堵”标签。
*   **输出**：`Score` (0-100) 及各项细分指标（含 `trafficCondition`）。

### 3.4 闭环路线生成算法 (Circuit Generation)
为满足“城市绕圈”必须回到原点的需求，采用以下策略生成闭环：

*   **折返模式 (Out-and-Back, A -> B -> A)**:
    1.  输入起点 A 和总目标距离 D。
    2.  计算单程距离 `R = D / 2`。
    3.  在 A 点周围随机选择一个方位角（或沿主干道方向），计算距离 R 处的坐标点 B。
    4.  分别调用百度骑行规划接口获取 `Route(A, B)` 和 `Route(B, A)`。
    5.  合并两条路线的 Steps、距离和耗时，形成完整的闭环数据。

*   **环线模式 (Loop, A -> B -> C -> A)** (Phase 2):
    1.  根据距离 D 构建一个多边形（如三角形），计算顶点 B 和 C。
    2.  分别规划 A->B, B->C, C->A。
    3.  合并结果。

MVP 阶段优先实现**折返模式**，确保逻辑简单且必定闭环。
